...
class Shader:
	def __init__(shader, path, type):
		...
		glShaderSource(shader.id, 1, byref(cast(c_char_p(source), POINTER(c_char))), c_int(len(source)))
...
rect_prog.enable() # THIS IS AWESOME!!!
#rect_prog.disable()
...
while 1:
	...
	draw_rect(vec2(0, 0), vec2(int(10*cos(left)), int(10*sin(left))), vec3(1, 0, 1))
